...
- **Tap-to-hear (text-to-speech):** speaker buttons read text aloud via the browser's Web Speech API — the word (English), and on each definition and example both the **English** and the **Sinhala** lines. English and Sinhala voices are chosen separately in Settings and persisted. Tap again to stop; tapping another cancels the first. Sinhala speakers appear only where the device has a Sinhala voice (e.g. Microsoft Edge's online voices; not on iOS), and can be turned off entirely with the **Sinhala speech** toggle in Settings.
- **Stacks of 20**, grouped into levels of 3. A **typed-recall mastery test** gates each level: you see a word's Sinhala meaning and type the English word (case/whitespace-insensitive, word forms accepted, typos are wrong). Missed words re-appear until you've recalled every one — only then does the next level unlock.
