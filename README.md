...
- **Tap-to-hear (text-to-speech):** speaker buttons read text aloud via the browser's Web Speech API — the word (English), and on each definition and example both the **English** and the **Sinhala** lines. English and Sinhala voices are chosen separately in Settings and persisted. Tap again to stop; tapping another cancels the first. Sinhala speakers appear only where the device has a Sinhala voice (e.g. Microsoft Edge's online voices; not on iOS).
